...
async function sendTriggeredPush(
  ...
  const header =
    triggered.length === 1
      ? `🔔 <b>${esc(triggered[0].alert.ticker)} alert</b>`
      : `🔔 <b>${triggered.length} alerts triggered</b>`;

  const body = triggered
    .map(
      ({ alert, current }) =>
        `• <b>${esc(alert.ticker)}</b> — ${esc(alert.description)}\n   now: ${esc(current)}`
    )
    .join("\n\n");

  return sendPush({ text: `${header}\n\n${body}` });
}
...
async function sendTriggeredEmail(
  ...
  const subject =
    triggered.length === 1
      ? `🔔 ${triggered[0].alert.ticker} alert triggered`
      : `🔔 ${triggered.length} portfolio alerts triggered`;

  const lines = triggered.map(
    ({ alert, current }) =>
      `• ${alert.ticker} — ${alert.description}\n   condition met (current: ${current})`
  );
  const text = `Your Portfolio Lens alert${triggered.length > 1 ? "s" : ""} fired:\n\n${lines.join("\n\n")}`;

  const htmlRows = triggered
    .map(
      ({ alert, current }) => `
      <tr>
        <td style="padding:8px 12px;font-weight:600;color:#111;">${alert.ticker}</td>
        <td style="padding:8px 12px;color:#333;">${alert.description}</td>
        <td style="padding:8px 12px;color:#0a7;white-space:nowrap;">${current}</td>
      </tr>`
    )
    .join("");
  const html = `
    <div style="font-family:system-ui,sans-serif;max-width:560px;">
      <h2 style="margin:0 0 4px;">🔔 Alert${triggered.length > 1 ? "s" : ""} triggered</h2>
      <p style="color:#666;margin:0 0 16px;">Conditions you set in Portfolio Lens have been met.</p>
