...
interface FullBacktestRequest {
  tickers: string[];
  weights: number[];        // decimal fractions, e.g. [0.5, 0.3, 0.2]
  startDate: string;        // "YYYY-MM-DD"
  endDate: string;          // "YYYY-MM-DD"
  initialCapital: number;
  rebalance: "none" | "monthly" | "quarterly" | "annual";
  benchmark: string | null; // "SPY", "QQQ", or null
}
...
function shouldRebalance(
  dates: string[],
  idx: number,
  mode: FullBacktestRequest["rebalance"]
): boolean {
...
export async function POST(req: NextRequest) {
  try {
    const body: FullBacktestRequest = await req.json();
    const { tickers, weights, startDate, endDate, initialCapital, rebalance, benchmark } = body;

    if (!tickers.length || tickers.length !== weights.length) {
    ...
    // Fetch price history for every ticker + benchmark in parallel
    ...
    for (const { sym, map } of rawPrices) priceMap[sym] = map;

    // Common trading days for portfolio tickers
    ...
    if (commonDates.length < 5) {
      return NextResponse.json({ error: "Insufficient price data for the selected period and tickers." }, { status: 400 });
    }

    // Initialize shares from weights + start prices
    ...
    function portfolioValue(date: string): number {
      return tickers.reduce((s, t) => s + (shares[t] ?? 0) * (priceMap[t][date] ?? 0), 0);
    }
    ...
    if (benchmark && priceMap[benchmark]) {
      ...
      // Find benchmark price on first common date (or nearest)
    ...
    const monthlyReturns = [...pMonths.entries()].map(([month, { start, end }]) => {
      ...
      return {
        ...
        ...(bEntry
          ? { benchmarkReturn: parseFloat((((bEntry.end - bEntry.start) / bEntry.start) * 100).toFixed(2)) }
          : {}),
      };
    });
    ...
    const excessMean    = dailyReturns.map((r) => r - riskFreeDaily).reduce((a, b) => a + b, 0) / dailyReturns.length;
    const excessStd     = Math.sqrt(
      dailyReturns.map((r) => r - riskFreeDaily).reduce((a, x) => a + (x - excessMean) ** 2, 0) / dailyReturns.length
    );
    ...
    console.error("Full backtest error:", err);
